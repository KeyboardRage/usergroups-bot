...
module.exports = {
	...
	options: [{
		name: "name",
		desc: "The name of the new voice channel",
		type: "STRING",
		required: true
	}, {
		name: "group-name",
	...
	permission: "group.new.channel.text",
	/**
	...
	async exec(payload) {
		...
		if (!payload.can("group.new.channel.text"))
			return payload.cant(`You don't have permission to create a new text channel for **${group.name}**.`);

		// Validate input
		let result = payload.api.groupApi.validate("tc-name", payload.arg("name"));
		...
		if (group.textChannelCount > payload.api.groupApi.config.max.hardChannelCount) return payload.reply(
			new MessageEmbed()
				.setColor("#cd1818")
		...
		try {
			...
			ch = await group.addChannel("text", payload.arg("name"), parent);
		} catch(err) {
		...
		return payload.reply(
			new MessageEmbed()
				...
				.setDescription(`A new text channel was created for **${group.name}**: <#${ch.id}>`)
		);
	}
}